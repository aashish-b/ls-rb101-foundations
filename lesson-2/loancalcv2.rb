...
loop do
  ...
  prompt("Hi #{given_name}, please enter your full loan amount (in USD $):")
  loan_amt = ''
  loop do
  ...
  zero_interest = to_float(loan_amtreal / loan_durreal)
  aprate_real = to_float(aprate)
  if aprate_real == 0
    puts "Your interest rate is 0 so your monthly payment will be
    $#{zero_interest} per month (in USD)."
    prompt("Do you want to use LoanCalc again? ('y' for yes)")
    run_again = gets.chomp
    return if run_again.downcase != 'y'
  else
    ...
    prompt("Your monthly payment is $#{monthly_pmt_final}
    per month (in USD $).")
